...
export default function ActualitesEtConseils() {
  ...
  return (
    <section className="py-8 md:pb-14 px-4 md:px-8 lg:px-50 bg-white">
      <h2 className="text-3xl md:text-4xl font-bold text-center text-gray-900 mb-4">Actualités et Conseils</h2>
      <p className="text-center text-gray-700 mb-8 md:mb-16 max-w-4xl mx-auto px-4">Pour tout savoir sur le photovoltaïque pour les particuliers et les professionnels, découvrez nos dernières actualités</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 max-w-7xl mx-auto">
        {articles.map((article, index) => (
          <div key={index} className="flex flex-col bg-white rounded-lg overflow-hidden">
            ...
            <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-3 px-2 md:px-0 line-clamp-2">{article.title}</h3>
            <Link href={article.href} className="text-blue-600 font-medium border-b-2 border-blue-600 pb-0.5 w-fit hover:border-blue-800 hover:text-blue-800 mt-auto mx-2 md:mx-0 mb-3">
              Lire l'article
            </Link>
          </div>
        ))}
