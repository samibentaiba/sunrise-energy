...
export default function ActualitesEtConseils() {
  ...
  return (
    <section className=" pb-5 px-50 bg-white">
      <section className="pb-14 bg-white">
        <h2 className="text-4xl font-bold text-center text-gray-900 mb-6">Actualités et Conseils</h2>

        <p className="text-center text-gray-700 mb-16 max-w-4xl mx-auto">Pour tout savoir sur le photovoltaïque pour les particuliers et les professionnels, découvrez nos dernières actualités</p>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {articles.map((article, index) => (
            <div key={index} className="flex flex-col">
              <div className="mb-4 overflow-hidden">
                <Image src={article.image} alt={article.title} width={324} height={256} className="object-cover w-full h-64" />
              </div>
              ...
              <Link href={article.href} className="text-blue-600 font-medium border-b-2 border-blue-600 pb-0.5 w-fit hover:border-blue-800 hover:text-blue-800 mt-auto">
                Lire l'article
              </Link>
            </div>
          ))}
